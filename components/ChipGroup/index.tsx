...
function ScheduleChipGroup() {
  ...
  const { queryParams } = useQueryParams();
  const store = useChipStore();
  const { scheduleSort, location, meter, date, depth, setScheduleSort, setLocation, setMeter, setDate, setDepth } =
    store;
        ...
        >
          {SCHEDULE_FILTERS[1].map(({ label, value }) => (
            <Chip
              ...
              key={value}
              value={value}
              checked={location?.includes(value)}
            >
              {label}
            </Chip>
          ))}
      ...
      <div className={$.group}>
        ...
        >
          {SCHEDULE_FILTERS[3].map(({ label, value }) => (
            <Chip classNames={{ label: $["mantine-Chip-label"] }} key={value} value={value} checked={date === value}>
              {label}
            </Chip>
          ))}
        </Chip.Group>
